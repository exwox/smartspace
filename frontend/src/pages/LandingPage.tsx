...
export default function LandingPage(){
  ...
  return <div className="public-landing min-h-screen overflow-x-hidden bg-slate-950 text-white">
    <section id="home" className="relative isolate min-h-screen overflow-hidden">
      ...
      <header className="future-nav sticky top-0 z-50 border-b border-cyan-300/10 bg-slate-950/60 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
          <Link to="/" className="flex items-center gap-3"><span className="brand-logo-shell"><img src="/brand/logo-api.png" alt="Logo" className="h-full w-full object-contain" /></span><div><p className="font-bold">SMART SPACE</p><p className="text-[10px] uppercase tracking-[.24em] text-sky-300/70">Raja Haji Fisabilillah · TNJ</p></div></Link>
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-78px)] max-w-7xl items-center px-5 pb-20 sm:px-8 lg:px-10">
        <div className="relative max-w-4xl" data-reveal><img src="/brand/logo-api.png" alt="" className="pointer-events-none absolute -right-10 -top-20 hidden h-44 w-44 opacity-10 blur-[.2px] lg:block" />
    ...
    {content.showCta&&<section className="motif-cta px-5 py-24 sm:px-8 lg:px-10"><div className="future-cta relative mx-auto max-w-5xl overflow-hidden rounded-[2.25rem] border border-cyan-200/15 bg-gradient-to-br from-cyan-300/[.10] to-blue-500/[.025] px-7 py-14 text-center" data-reveal><img src="/brand/logo-api.png" alt="" className="pointer-events-none absolute -right-8 -top-10 h-36 w-36 opacity-[.08]" /><Eyebrow>Explore Commercial Space</Eyebrow><h2 className="mx-auto mt-4 max-w-3xl text-3xl font-black sm:text-5xl">{content.ctaTitle}</h2><p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-slate-300">{content.ctaDescription}</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link to="/map" className="cta-primary">{content.heroPrimaryButton} →</Link><Link to="/tracking" className="cta-secondary">Sudah mengajukan? Cek tiket</Link></div></div></section>}

    {content.showFooter&&<footer className="motif-footer border-t border-cyan-300/10 px-5 py-8 text-xs text-slate-400 sm:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><img src="/brand/logo-api.png" alt="Logo" className="h-9 w-9 object-contain opacity-90"/><p>{content.footerText}</p></div><div className="flex gap-4"><a href="#home">Beranda</a><Link to="/map">Peta Ruang</Link><Link to="/tracking">Tracking</Link></div></div></footer>}
  </div>
}
